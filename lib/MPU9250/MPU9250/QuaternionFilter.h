...
class QuaternionFilter {
    ...
    bool useZeta = false;  // Controls whether to use zeta (gyroscope drift compensation)
    float w_bx = 0.0f;     // Accumulated error (Bias) X
    float w_by = 0.0f;     // Accumulated error (Bias) Y
    float w_bz = 0.0f;     // Accumulated error (Bias) Z
...
public:
    ...
    void madgwick(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q) {
        ...
        if (useZeta) {
            ...
            w_bx += zeta * s_w_x * deltaT;
            w_by += zeta * s_w_y * deltaT;
    ...
    void mahony(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q) {
        ...
        if (tmp > 0.0) {
            ...
            // Compute feedback only if magnetometer measurement valid 
            double m_norm = mx * mx + my * my + mz * mz;
            if (m_norm > 0.0) {
                recipNorm = 1.0 / sqrt(m_norm);
                mx *= recipNorm;
                ...
                // Reference direction of Earth's magnetic field (Estimated)
                float hx = mx * q[0] * q[0] - 2.0f * q[0] * my * q[3] + 2.0f * q[0] * mz * q[2] + mx * q[1] * q[1] + 2.0f * q[1] * my * q[2] + 2.0f * q[1] * mz * q[3] - mx * q[2] * q[2] - mx * q[3] * q[3];
                float hy = 2.0f * q[0] * mx * q[3] + my * q[0] * q[0] - 2.0f * q[0] * mz * q[1] + 2.0f * q[1] * mx * q[2] - my * q[1] * q[1] + my * q[2] * q[2] + 2.0f * q[2] * mz * q[3] - my * q[3] * q[3];
                float bx = sqrt(hx * hx + hy * hy);
                float bz = -2.0f * q[0] * mx * q[2] + 2.0f * q[0] * my * q[1] + mz * q[0] * q[0] + 2.0f * q[1] * mx * q[3] - mz * q[1] * q[1] + 2.0f * q[2] * my * q[3] - mz * q[2] * q[2] + mz * q[3] * q[3];

                // Estimated direction of magnetic field
                float wx = 2.0f * bx * (0.5f - q[2] * q[2] - q[3] * q[3]) + 2.0f * bz * (q[1] * q[3] - q[0] * q[2]);
                float wy = 2.0f * bx * (q[1] * q[2] - q[0] * q[3]) + 2.0f * bz * (q[0] * q[1] + q[2] * q[3]);
                float wz = 2.0f * bx * (q[0] * q[2] + q[1] * q[3]) + 2.0f * bz * (0.5f - q[1] * q[1] - q[2] * q[2]);

                // Error is cross product between estimated direction and measured direction of magnetic field
                ex += (my * wz - mz * wy);
                ey += (mz * wx - mx * wz);
                ez += (mx * wy - my * wx);
            }
